...
int semicommand = 0;
	for (int i = 0; i < NUM_OF_COMMANDS; i++)
	{
	 ...
	 for (int i = 0; i < NUM_OF_COMMANDS; i++) {
	        int counter = 0;  // Counter to track matched characters
	        int arg_len = strlen(arguments[0]);  // Length of the word in arguments[0]

	        // Loop through each character in arguments[0]
	        for (int j = 0; j < arg_len; j++) {
	            // Check if the current character of arguments[0] is found in commands[i].name
	            if (strchr(commands[i].name, arguments[0][j]) != NULL) {
	                counter++;  // Increment the counter when a match is found
	            }
	        }

	        // If all characters of arguments[0] have been matched, it's a partial match
	        if (counter == arg_len) {
	            semicommand = 1;
