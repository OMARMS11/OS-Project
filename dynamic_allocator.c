...
void initialize_dynamic_allocator(uint32 daStart, uint32 initSizeOfAllocatedSpace)
{
	...
	//setting the address for the borders of the heap and the header and footer for the block
	uint32* BegHeap = (uint32*)daStart;
	uint32* EndHeap = (uint32*)(daStart + initSizeOfAllocatedSpace - sizeof(int));
	uint32* blkHeader = (uint32*)(daStart + sizeof(int));
	uint32* blkFooter = (uint32*)(daStart + initSizeOfAllocatedSpace - 2*sizeof(int));
	//setting the value for each block
	*BegHeap = 1;
	*EndHeap = 1;
	...
	*blkFooter = initSizeOfAllocatedSpace - 2 * sizeof(int);
	...
	cprintf("the first element : %d",(uint32)LIST_FIRST(&freeBlocksList));
